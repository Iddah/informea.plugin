...
<div class="wrap">
	<div id="breadcrumb">
		You are here:
		<a href="<?php echo bloginfo('url');?>/wp-admin/admin.php?page=informea_national_plans">Manage national plans</a>
		&raquo;
		<a href="<?php echo bloginfo('url');?>/wp-admin/admin.php?page=informea_national_plans&act=add_national_plan">Add new national plan</a>
	</div>
	...
	<form action="" method="post" id="sel_form">
		...
		<table>
			...
			<tr>
				...
				<td>
					<input type="text" size="60" id="submission" name="submission" value="<?php if (!$page_data->success) echo $page_data->get_value('submission');?>" />
				</td>
			</tr>
			<tr>
				<td><label for="document_url">Document URL *</label></td>
				<td>
					<input type="text" size="60" id="document_url" name="document_url" value="<?php if (!$page_data->success) echo $page_data->get_value('document_url');?>" />
				</td>
			</tr>
		</table>
		<p> * - Required field(s) </p>
		<p class="submit">
			<input name="actioned" type="submit" class="button-primary" value="<?php esc_attr_e('Add national plan'); ?>" />
		</p>
	</form>
</div>
