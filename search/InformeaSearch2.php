...
class InformeaSearch2 extends AbstractSearch {
    ...
    function /* InformeaSearchResult2 */
    search($debug = false) {
        $ret = null;
    ...
    function remove_untagged_items(&$interim) {
        ...
        foreach ($tagged_decisions_paragraphs_obs as $ob) {
            $tagged_decisions_paragraphs_ids[] = $ob->id;
        }

        $tagged_decisions_obs = $this->find_tagged_decisions();
        $tagged_decisions_ids = array();
        foreach ($tagged_decisions_obs as $ob) {
            $tagged_decisions_ids[] = $ob->id;
        }
        ...
        foreach ($tagged_treaty_paragraphs_obs as $ob) {
            $tagged_treaty_paragraphs_ids[] = $ob->id;
        }

        $tagged_treaty_articles_obs = $this->find_tagged_treaty_articles();
        $tagged_treaty_articles_ids = array();
        foreach ($tagged_treaty_articles_obs as $ob) {
            $tagged_treaty_articles_ids[] = $ob->id;
        }

        $tagged_treaties_obs = $this->find_tagged_treaties();
        $tagged_treaties_ids = array();
        foreach ($tagged_treaties_obs as $ob) {
            $tagged_treaties_ids[] = $ob->id;
        }
        ...
        foreach ($interim->treaties as $treaty) {
            foreach ($treaty->articles as $article) {
                ...
                if (empty($article->paragraphs)) { // If the article has no paragraphs, look for article tags
                    ...
                    if (!$atagged) {
                        unset($treaty->articles[$article->id]);
                    } // Remove, article is not tagged
                }
            }
