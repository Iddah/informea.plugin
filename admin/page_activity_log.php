<div class="wrap">
    ...
    <p>
        <?php if ($limit == 'all') { ?>
            Showing all entries.
            Click <a
                href="<?php echo get_bloginfo('url'); ?>/wp-admin/admin.php?page=informea_activity_log&limit=<?php echo $limit; ?>&order=<?php echo $order; ?>&asc=<?php echo $ascension; ?>&limit=100">here</a> to show only last 100 entries.
        <?php } else { ?>
            Showing latest 100 entries.
            Click <a
                href="<?php echo get_bloginfo('url'); ?>/wp-admin/admin.php?page=informea_activity_log&limit=<?php echo $limit; ?>&order=<?php echo $order; ?>&asc=<?php echo $ascension; ?>&limit=all">here</a> to show full log (it may be very long).
        <?php } ?>
    </p>
    <table class="widefat fixed">
        ...
        <?php foreach ($records as $record) { ?>
            <tr>
                ...
                <td>
                    <pre><?php echo esc_attr($record->description); ?></pre>
                </td>
            </tr>
        <?php } ?>
        </tbody>
