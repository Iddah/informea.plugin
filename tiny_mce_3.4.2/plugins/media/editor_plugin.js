(function () {
	...
	tinymce.create("tinymce.plugins.MediaPlugin", {init: function (n, j) {
		...
		return this.editor.serializer.serialize(this.dataToImg(i, j), {force_absolute: j})
		...
		function q(i, G) {
			...
			if (H) {
				...
				L = "";
				J = o.getParam("flash_video_player_flashvars", {url: "$url", poster: "$poster"});
				...
				I = o.getParam("flash_video_player_params", {allowfullscreen: true, allowscriptaccess: true});
		...
		if (p.name === "Iframe") {
			...
			v.attr({style: z, src: D.params.src});
			x.replace(v);
			return
		}
		if (this.editor.settings.media_use_script) {
			...
			w = new b("#text", 3);
			w.value = "write" + p.name + "(" + g.serialize(tinymce.extend(D.params, {width: x.attr("width"), height: x.attr("height")})) + ");";
			v.append(w);
			x.replace(v);
			return
		}
		if (p.name === "Video" && D.video.sources[0]) {
			A = new b("video", 1).attr(tinymce.extend({id: x.attr("id"), width: x.attr("width"), height: x.attr("height"), style: z}, D.video.attrs));
			if (D.video.attrs) {
		...
		if (D.params.src) {
			...
			E = new b("object", 1).attr({id: x.attr("id"), width: x.attr("width"), height: x.attr("height"), style: z});
			tinymce.each(d, function (i) {
				if (D[i] && i != "type") {
					E.attr(i, D[i])
				}
			...
			for (F in D.params) {
				...
				r.attr({name: F, value: w});
				E.append(r)
			}
			if (this.editor.getParam("media_strict", true)) {
				E.attr({data: D.params.src, type: p.mimes[0]})
			} else {
				E.attr({classid: "clsid:" + p.clsids[0], codebase: p.codebase});
				j = new b("embed", 1);
				j.shortEnded = true;
		...
		if (H === "video") {
			...
			x = y.attr("src");
			if (x) {
				E.video.sources.push({src: s.call(m, x, "src", "video")})
			}
			k = A.getAll("source");
		...
		G.attr({id: u, "class": "mceItemMedia mceItem" + (n || "Flash"), style: B, width: w || "320", height: t || "240", "data-mce-json": g.serialize(E, "'")})
	}});
